...
public class SequenceAligner {
    ...
    private void fillCache() {
        ...
        for (int row = 1; row <= n; row++) {
            for (int col = 1; col <= m; col++) {
                int diag = cache[row-1][col-1].getScore() + judge.score(x.charAt(row - 1),y.charAt(col - 1));
                int left = cache[row][col-1].getScore() + judge.getGapCost();
                int up = cache[row-1][col].getScore() + judge.getGapCost();

                int bestScore = Integer.max(diag,Integer.max(left,up));
                Result best = new Result(bestScore, Direction.DIAGONAL);;
                if(diag == bestScore){
                    best = new Result(diag, Direction.DIAGONAL);
                }
                else if(left == bestScore){
    ...
    private void traceback() {
        //insert: gap in string one x
        //delete: gap in string two y
        //left or up: is a gap to align string

        Result loc = cache[n][m];
        Direction nextDirection;
        Result goal = cache[0][0];
        alignedY = "";
        alignedX = "";
        ...
        while(loc != goal) {
            loc = cache[n][m];
            nextDirection = loc.getParent();
            if (nextDirection == Direction.DIAGONAL) {
                n = n -1;
                m = m -1;
                alignedX += x.charAt(n);
                alignedY += y.charAt(m);
        ...
        inputX.append(alignedX);
        inputY.append(alignedY);
        inputX.reverse();
        inputY.reverse();
        alignedX = inputX.toString();
        alignedY = inputY.toString();
    }
